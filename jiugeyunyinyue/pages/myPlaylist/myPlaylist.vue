...
<view class="yunshi">查看运势详情></view>
 ...
 <view class="recommend-list" @touchmove="handlerStart" >
  <view class="listHeader" id="listH" :class="top?'top':'no'" >
   ...
   <songList :songList="myPlaylist" :from="'playlist'" :isFrom="'myPlaylist'"></songList>
 </view>
    </view>
  ...
  export default {
    ...
    methods:{
     ...
     //    "节点离页面顶部的距离为"  data.top
      ...
      //获取每日推荐歌曲列表信息
      getMyPlaylist(){
        uni.$http('/user/playlist',{uid:this.userId}).then(({data:{playlist}})=>{
          console.log(playlist);
          this.myPlaylist = playlist
          this.myPlaylist = this.myPlaylist.map((item)=>{
                  return{
                    // songIndex = index
                    songName:item.name,
                    songId:item.id,
